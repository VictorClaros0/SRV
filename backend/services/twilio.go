...
func NewTwilioClient(accountSID, authToken, messagingSID, verifiedNumber string) *TwilioClient {
	if accountSID == "" || authToken == "" || messagingSID == "" {
		log.Println("Twilio no configurado: TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN / TWILIO_MESSAGING_SERVICE_SID ausentes")
		return nil
	}
	return &TwilioClient{
...
func (t *TwilioClient) SendSMS(to, body string) error {
	...
	if resp.StatusCode >= 400 {
		...
		return fmt.Errorf("twilio: error HTTP %d: %s", resp.StatusCode, string(b))
	}
	return nil
}

// SendConfirmation envía un SMS de confirmación. En cuentas trial usa VerifiedNumber como destino.
func (t *TwilioClient) SendConfirmation(toNumber, mensaje string) {
	...
	if dest == "" {
		log.Printf("Twilio: no hay número destino para confirmación")
		return
	}
	if err := t.SendSMS(dest, mensaje); err != nil {
		log.Printf("Twilio: error enviando confirmación a %s: %v", dest, err)
	} else {
