...
func main() {
	...
	smsConfigRepo := repository.NewSMSConfigRepository(db, cfg.SMSDefaultNumbers)
	...
	smsInboundH := handlers.NewSMSInboundHandler(rrvActaRepo, actaRepo, eventoRepo, smsConfigRepo)
	scanH := handlers.NewScanHandler(rrvActaRepo, eventoRepo)
	...
	rrv := r.Group("/api/rrv")
	{
		...
		rrv.POST("/webhook/sms", rrvH.WebhookSMS) // Twilio webhook
		rrv.GET("/actas", rrvH.GetAll)
		rrv.GET("/actas/:acta_id", rrvH.GetByID)
		rrv.GET("/eventos", rrvH.GetEventos)

		// Webhook público para SMS Forwarder app (sin prefijo /v1 para compatibilidad)
		rrv.POST("/sms/inbound", smsInboundH.InboundSMS)
	}

	// Rutas SMS bajo /api/v1 para AppWeb (httpClientRrv base = /api/v1)
	v1sms := v1.Group("/rrv/sms")
	{
		v1sms.GET("/historial", smsInboundH.GetHistorial)
		v1sms.GET("/numeros", smsInboundH.ListNumeros)
		v1sms.POST("/numeros", smsInboundH.AddNumero)
		v1sms.DELETE("/numeros/:numero", smsInboundH.RemoveNumero)
	}

	// ── Scanner Móvil ─────────────────────────────────────────────────────────
	v1.GET("/rrv/scan/check", scanH.CheckMesa)
	v1.POST("/rrv/scan", scanH.SaveScan)
	v1.POST("/rrv/scan/observada", scanH.SaveObservada)
	v1.POST("/rrv/scan/rechazada", scanH.SaveRechazada)
