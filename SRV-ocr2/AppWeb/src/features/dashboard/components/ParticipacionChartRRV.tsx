...
export const ParticipacionChartRRV = ({ filters }: Props = {}) => {
    const f = filters ?? { departamento: "", municipio: "", provincia: "", recinto: "", mesa: "" };
    const { data, isLoading } = useQuery({
        queryKey: ["rrv-participacion-filt", f.departamento, f.municipio, f.provincia, f.recinto, f.mesa],
        queryFn: async () => {
            const { call } = rrvService.getKPIs(f);
    ...
    const validos = data.votosValidos ?? 0;
    const nulos = data.votosNulos ?? 0;
    const blanco = data.votosBlancos ?? 0;
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow h-96 flex flex-col">
            <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-1">
                Participación electoral (conteo rápido)
            </h3>
