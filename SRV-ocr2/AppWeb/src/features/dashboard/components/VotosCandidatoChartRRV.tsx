...
export const VotosCandidatoChartRRV = ({ filters }: Props = {}) => {
    const f = filters ?? { departamento: "", municipio: "", provincia: "", recinto: "", mesa: "" };
    const { data, isLoading } = useQuery({
        queryKey: ["rrv-votosCandidato", f.departamento, f.municipio, f.provincia, f.recinto, f.mesa],
        queryFn: async () => {
            const { call } = rrvService.getVotosCandidato(f);
    ...
    const totalVotos = cands.reduce((a, b) => a + b.votos, 0);

    const chartData = cands.map((it, i) => ({
        candidato: it.nombre,
        votos: it.votos,
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow h-96 flex flex-col">
            <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-1">
                Votos por candidato (conteo rápido)
            </h3>
            <p className="text-[11px] text-gray-400 mb-2">
                Total candidatos: {totalVotos.toLocaleString("es-BO")}
            </p>
